...
def __init__(base_path, config):
    ...
    for page in config['bottomMenu']:
        rendered_galleries[page] = None

def interpret_config(page, config, source_path, source, file_name, default, configPage, children, parents, index, bodyhtml):
    if 'images' in page:
        changed = False
        if discover:
        ...
        body = TEMPLATES['gallery'].replace('{{title}}', page['pageTitle'])
        ...
        for image in page['images']:
            ...
            if isinstance(image, dict):
                ...
                if path == 'n':
                    ...
                    if image is not None and '/' in image and '.com' not in image:
                        ...
                        image = image[1]
                    body = TEMPLATES['category'].replace('{{title}}', page['pageTitle'])
                    body = body.replace('{{body}}', page['body'])
            ...
            if 'isCategory' not in page:
                if not clear or os.path.isfile(os.path.join(source_path, image)) and default['images'].count(image) < 2:
        ...
        if 'isCategory' in page and page['isCategory'] and configPage in config['bottomMenu']:
            rendered_galleries[configPage] = page['body']
        else:
...
def render_page(page, config):
    global rendered_gallery
    if rendered_gallery is None:
        rendered_gallery = ''.join(rendered_galleries[page] for page in config['bottomMenuPages'])

    page['body'] = page['body'].replace('{{gallery}}', rendered_gallery)
